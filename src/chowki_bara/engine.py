...
class Engine:
    ...
    def move_piece(self, piece_index):
        ...
        if self.board.pending_player[piece_index].is_unlocked or not self.has_won(self.pending_player):
            self.board.pending_player[piece_index].move_steps(self.board.current_shell_value)
        else:
            if self.board.current_shell_value in self.shells.opens_pieces_on:
                self.board.pending_player[piece_index].is_unlocked = True
        
        # (2.3.2) Updating castled_pieces values        
        if pending_player.pieces[piece_index].current_pos == (0, 0):
            if not hasattr(pending_player, castled_pieces):
                pending_player.castled_pieces = 1
            else:
                pending_player.castled_pieces += 1    
                    
        # (2.3.3) Post movement steps
        ...
        if not self.board.current_shell_value in self.shells.rewarding_numbers:
            self.board.pending_player = (self.board.pending_player + 1) % len(self.players) # Rotating Player if the numbers aren't rewarding
            
        self.board.current_shell_value = None # Resetting shell value
        
        return self.board.last_player[self.board.last_player.index].current_pos
    
    # (2.4) A method to investigate if a player has won
    def has_won(self, player):
        if value := all(piece.current_pos == (0, 0) for piece in player.pieces):
            return value
